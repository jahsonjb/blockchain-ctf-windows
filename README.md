...
**GitHub repo:** _<insert your repository URL here>_
...
- **PowerShell** — built in to Windows (for simple decoding and hashing output checks)

---

## How to Solve (Step-by-Step)

All steps assume the files are together (e.g., `C:\CTF`) and you have read/write permissions.

### Block 1 — Stego → Base64

1) **Extract the hidden file**

Open **OpenStego** → **Data Extraction**  
...
You will use this value as the Block-2 passphrase.

---

### Block 2 — Stego → SHA-3-256 key

1. **Extract the hashing instruction**
...
* In **CyberChef**, clear any prior recipe.
* Add operation **`SHA3-256`** (ensure it’s SHA3, not Keccak).
...
* In **CyberChef**, clear any prior recipe, then add **AES Decrypt**.
* Set:
...
Anyone can verify they are using the exact same images by checking **SHA-256**.
...
* **AES decryption shows gibberish or fails**
  ...
  * Be sure you’re decrypting the extracted ciphertext (e.g., `3.enc`).

---

## (Optional) How This Was Built

> This section documents how the artifacts were authored. You do **not** need to repeat these steps to solve.
...
* **Block 2**
  ...
  * Embedded into a PNG cover via **OpenStego → Data Hiding**, password `Cyb3rCh4ll3ng3`, output `SecPlus2.png`.
...
* **Block 3**
  ...
  * Embedded `3.enc` into a PNG cover via **OpenStego → Data Hiding**, password `ZeroTrust`, output `SecPlus3.png`.
...
* `02_Block1_Extract.png` — OpenStego Data Extraction for Block 1 (success dialog visible)
* `03_Block1_Base64.png` — PowerShell Base64 decode → `Cyb3rCh4ll3ng3`
* `04_Block2_Extract.png` — OpenStego Data Extraction for Block 2 (success; `2.txt` open)
* `05_Block2_SHA3.png` — CyberChef SHA3-256 of `ZeroTrust` with the hex digest visible
* `06_Block3_Extract.png` — OpenStego Data Extraction for Block 3 (success; ciphertext present)
